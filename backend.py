...
async def generate_task(req: TaskRequest):
    ...
    The "answers" array MUST be exactly this list: {json.dumps(req.words)}.
...
async def check_answer(req: dict):
    prompt = f"Check if the word '{req.get('user_word', '')}' is a valid synonym or exactly matches '{req.get('correct_word', '')}' in English. Return ONLY a valid JSON object: {{\"is_correct\": true or false}}"
